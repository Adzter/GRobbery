...
function ENT:Draw()
	...
	if self:GetNWInt( "drillTimer" ) and self:GetNWBool( "isDrilling" ) then
		cam.Start3D2D( self:LocalToWorld(Vector( 3, 12, 1 )), ang, 0.1)
			if self:GetNWBool( "isJammed" ) then
				draw.RoundedBox( 0, -128, 0, 75, 25, drillConfig.jammedBackgroundColor )
				draw.RoundedBox( 2, -53, -45, 150, 125, drillConfig.jammedBackgroundColor )
				
				draw.RoundedBox( 0, -40, 40, 125, 20, drillConfig.jammedProgressBarBackgroundColor )
				draw.RoundedBox( 0, -40, 40, self:GetNWInt( "drillTimer" )/3, 20, drillConfig.jammedProgressBarColor )
				
				draw.SimpleText( drillConfig.drillingText, "DermaLarge", -35, -30, drillConfig.jammedTextColor, 0, 0)
				draw.SimpleText( self:GetNWInt( "drillTimer" ), "DermaLarge", 0, 0, drillConfig.jammedTextColor, 0, 0)
			else
